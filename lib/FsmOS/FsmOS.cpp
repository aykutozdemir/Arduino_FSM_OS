...
uint16_t Scheduler::getFreeMemory() const
{
    ...
    // More accurate: use SP register directly for better precision
    ...
    uint16_t stack_ptr = (uint16_t)SP;
    // Ensure we don't underflow
    return (stack_ptr > heap_end) ? (stack_ptr - heap_end) : 0;
#else
    // Fallback for non-AVR platforms
    extern char *__brkval;
    char v;
    return (uint16_t)(&v - (__brkval == 0 ? (char *)&v : __brkval));
#endif
}
...
bool Scheduler::getSystemMemoryInfo(SystemMemoryInfo &info)
{
...
#if defined(__AVR__)
    // Calculate total RAM dynamically using linker symbols
    extern char __data_start;
    extern char __heap_start;
    // Total RAM = RAMEND - RAMSTART (or __data_start for most AVR)
    // For AVR, RAM typically starts at 0x100 (256) and ends at RAMEND
    // More accurate: RAMEND - (uint16_t)&__data_start + 1
    info.totalRam = (uint16_t)(RAMEND - (uint16_t)&__data_start + 1);
#else
    info.totalRam = 2048;    // Default fallback
#endif
    ...
    info.heapSize = 0;
#endif
    // Calculate largest free block (simplified - largest contiguous free space)
#if defined(__AVR__)
    extern char *__brkval;
    char v;
    uint16_t heap_end = (uint16_t)(__brkval == 0 ? (uint16_t)&__heap_start : (uint16_t)__brkval);
    uint16_t stack_ptr = (uint16_t)SP;
    // Largest block is the free RAM (simplified - doesn't account for heap fragmentation)
    info.largestBlock = (stack_ptr > heap_end) ? (stack_ptr - heap_end) : 0;
    // Heap fragmentation: simplified - count if heap is used
    info.heapFragments = (__brkval != 0) ? 1 : 0;  // Basic: 0 or 1 (fragmented)
#else
    info.largestBlock = 0;
    info.heapFragments = 0;
#endif
#if defined(__AVR__)
    ...
    // Flash usage calculation - try runtime linker symbols first, then build-time constants
#if defined(__AVR__)
    // On AVR, we can use program memory addresses to calculate flash usage
    // Flash addresses start at 0x0000, but we need to account for bootloader
    // Try different declaration methods for linker symbols
    extern void *__vectors_end __attribute__((weak));
    extern void *__ctors_end __attribute__((weak));
    extern void *__dtors_end __attribute__((weak));
    
    uint16_t flash_used = 0;
    uint16_t flash_total = 0;
    
    // Get total flash from build defines or use MCU-specific defaults
    #ifdef FSMOS_FLASH_SIZE
        flash_total = FSMOS_FLASH_SIZE;
    #elif defined(__AVR_ATmega328P__) || defined(__AVR_ATmega328__)
        flash_total = 32768;  // 32KB for ATmega328P
    #elif defined(__AVR_ATmega2560__)
        flash_total = 262144;  // 256KB for ATmega2560
    #elif defined(__AVR_ATmega1280__)
        flash_total = 131072;  // 128KB for ATmega1280
    #elif defined(__AVR_ATmega168__)
        flash_total = 16384;   // 16KB for ATmega168
    #else
        flash_total = 32768;  // Default 32KB
    #endif
    
    // Try to estimate flash used using available linker symbols
    // Use weak symbols so code compiles even if symbols don't exist
    // Calculate flash used by finding the highest program memory address
    uintptr_t vectors_end_addr = (uintptr_t)&__vectors_end;
    uintptr_t ctors_end_addr = (uintptr_t)&__ctors_end;
    uintptr_t dtors_end_addr = (uintptr_t)&__dtors_end;
    
    // Check if symbols are valid (weak symbols will be NULL if not found)
    if (dtors_end_addr != 0 && dtors_end_addr > vectors_end_addr)
    {
        // Estimate: vectors + code + ctors + dtors
        // This gives a lower bound (doesn't include all PROGMEM data)
        flash_used = (uint16_t)(dtors_end_addr - 0x0000);
    }
    else if (ctors_end_addr != 0 && ctors_end_addr > vectors_end_addr)
    {
        flash_used = (uint16_t)(ctors_end_addr - 0x0000);
    }
    else if (vectors_end_addr != 0)
    {
        flash_used = (uint16_t)(vectors_end_addr - 0x0000);
    }
    else
    {
        // Symbols not available, will fall back to build-time constants or 0
        flash_used = 0;
    }
    
    // Prefer build-time constants if available (more accurate)
    #ifdef BUILD_FLASH_USED
        if (BUILD_FLASH_USED > 0)
        {
        ...
        {
            info.flashUsed = flash_used;
            info.flashFree = (flash_total > flash_used) ? (flash_total - flash_used) : 0;
        }
    #else
        info.flashUsed = flash_used;
        info.flashFree = (flash_total > flash_used) ? (flash_total - flash_used) : 0;
    #endif
#else
    // Non-AVR: use build-time constants only
    #ifdef BUILD_FLASH_USED
        info.flashUsed = BUILD_FLASH_USED;
        info.flashFree = BUILD_FLASH_FREE;
    #else
