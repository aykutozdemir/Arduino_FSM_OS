...
// Example controller that uses Task
class ExampleController : public Task
{
public:
  enum State
  {
  ...
  ExampleController() : Task(F("ExampleController")), m_state(IDLE), m_counter(0) 
  {
  ...
  {
    Serial.print(F("ExampleController initialized"));
    Serial.println();
  }
...
void setup()
{
  ...
  // Initialize OS and add controller task
  OS.begin();
  OS.add(&controller);
  controller.start();
