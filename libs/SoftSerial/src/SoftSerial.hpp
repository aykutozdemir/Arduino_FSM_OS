                                                              ...
                                                              const ParityMode parity)
{
  ...
  {
    OS.logMessage(nullptr, Scheduler::LOG_ERROR, PGMT(SOFT_SERIAL_BAUD_TOO_HIGH));
    return;
  }
...
void SoftSerial<RX_BUFFER_SIZE, TX_BUFFER_SIZE>::loop()
{
  ...
  for (uint8_t i = 0; (i < 8) && !m_rxQueue.isFull(); i++)
  {
    ...
    {
      OS.logMessage(nullptr, Scheduler::LOG_ERROR, PGMT(SOFT_SERIAL_START_BIT_ERR));
      hasError = true;
    }
    ...
    if (!hasError)
    {
      for (uint8_t s = 0; s < m_flags.stopBitCount; s++)
      {
        ...
        if ((frame & (1 << s)) == 0)
        {
          OS.logMessage(nullptr, Scheduler::LOG_ERROR, PGMT(SOFT_SERIAL_STOP_BIT_ERR));
          hasError = true;
          break;
        }
      }
    ...
    if (m_flags.parityType != NONE)
    {
      ...
      {
        OS.logMessage(nullptr, Scheduler::LOG_ERROR, PGMT(SOFT_SERIAL_PARITY_ERR));
        continue;
      }
    }
    ...
    {
      OS.logMessage(nullptr, Scheduler::LOG_ERROR, PGMT(SOFT_SERIAL_RX_BUF_FULL));
    }
  }
